...
function control_c {
    echo "[worsica_atmospheric_correction_service_uc.sh] HALT! Caught ctrl+c; Clean up and Exit \n"
    if [[ ${USERCHOSEN_STATE} == *'converting'* ]] ; then
    	USERCHOSEN_STATE='error-converting'
    fi
    ...
    echo "[worsica_atmospheric_correction_service_uc.sh] ${SERVICE}-user${USER_ID}-roi${ROI_ID}-s${USERCHOSEN_ID} state: ${USERCHOSEN_STATE}"
    exit 1
}
...
#[Converting]
if [[ ${USERCHOSEN_STATE} == *'downloaded'* ]] || [[ ${USERCHOSEN_STATE} == *'error-converting'* ]] ; then
	create_folder ${SERVICE} ${USER_ID} ${ROI_ID} ${USERCHOSEN_ID} ${USERCHOSEN_STATE}
	echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] ${SERVICE}-user${USER_ID}-roi${ROI_ID}-s${USERCHOSEN_ID} state: converting"
	echo "==========================Converting=============================="
	#echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] Download content from the nextcloud ${NEXTCLOUD_REMOTE_PATH}/${ROI_SIMULATION_FOLDER}"
    ...
    pull_imageset ${SERVICE} ${USER_ID} ${ROI_ID} ${USERCHOSEN_ID} ${NEW_IMAGESET_NAME}
    cd ${PATH_TO_PRODUCTS}
    if ([ -f ${NEW_IMAGESET_NAME}.zip ]); then
        echo "[worsica_atmospheric_correction_service.sh] [Converting] File exists, skip conversion!"
        echo "[worsica_atmospheric_correction_service.sh] [Converting] ${SERVICE}-user${USER_ID}-roi${ROI_ID}-s${USERCHOSEN_ID} state: converted"
        rm -rf ${PATH_TO_PRODUCTS}/${IMAGESET_NAME}.zip
        rm -rf ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}.zip 
        exit 0
    else
        ...
        if (cd ${PATH_TO_PRODUCTS} && ([ -d ${DATASTRIP_PATH}/QI_DATA ] || mkdir ${DATASTRIP_PATH}/QI_DATA) && L2A_Process ${IMAGESET_NAME}.SAFE --output_dir ${NEW_IMAGESET_NAME}.SAFE); then
            rm -rf ${PATH_TO_PRODUCTS}/${IMAGESET_NAME}.zip
            ...
            if (cd ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}.SAFE && dirname $(find . -name MTD_MSIL2A.xml)); then
                ...
                echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] Go to ${L2ASAFE_PATH}"
                cd ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}.SAFE/${L2ASAFE_PATH}  
                echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] Move contents from ${L2ASAFE_PATH} to top folder ${NEW_IMAGESET_NAME}.SAFE"
                mv * ../ && cd ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}.SAFE
                echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] ...and remove it"
                rm -rf ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}.SAFE/${L2ASAFE_PATH}
                cd ${PATH_TO_PRODUCTS}
                echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] ZIP ./${NEW_IMAGESET_NAME}.SAFE"
                zip -r ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}.zip ${NEW_IMAGESET_NAME}.SAFE       
                push_imageset ${SERVICE} ${USER_ID} ${ROI_ID} ${USERCHOSEN_ID} ${NEW_IMAGESET_NAME}
                #remove folder (either .safe or not)
                rm -rf ${PATH_TO_PRODUCTS}/${IMAGESET_NAME}*
                rm -rf ${PATH_TO_PRODUCTS}/${NEW_IMAGESET_NAME}*
                echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] ${SERVICE}-user${USER_ID}-roi${ROI_ID}-s${USERCHOSEN_ID} state: converted"
                exit 0
            else
                echo "New .SAFE folder generated by the sen2cor not found"
                echo "[worsica_atmospheric_correction_service_uc.sh] [Converting] ${SERVICE}-user${USER_ID}-roi${ROI_ID}-s${USERCHOSEN_ID} state: error-converting"
                exit 1
            fi		
        else
            #remove imageset to free up the workspace folder
