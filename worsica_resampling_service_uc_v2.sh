...
if [[ ${USERCHOSEN_STATE} == *'downloaded'* ]] || [[ ${USERCHOSEN_STATE} == *'error-resampling'* ]] || [[ ${USERCHOSEN_STATE} == *'converted'* ]] ; then
	...
	if (cd ${PATH_TO_PRODUCTS} && python3 -u ${WORSICA_FOLDER_PATH}/worsica_sentinel_script_v5_resampling.py ${IMAGESET_NAME} ${USERCHOSEN_NAME} "${GEOJSON_POLYGON}" ${SERVICE} "${BANDS}"); then
