...
def run_script(args):
    ...
    try:
        ...
        print('---------------------------------------------')
        print('-- 1) Calculate 2nd deriv ' + args[3] +
              ' using imageset ' + args[1] + ' (' + args[2] + ') --')
        print('---------------------------------------------')
        waterIndexes = args[2].split(',')
        fromAnomaly = (args[3] == 'by_anomaly')  # by_anomaly or by_index
        if (fromAnomaly):
            # ./ldYYY/merged_resampled_xxxxxxxxx_wi[_anomaly].tif
            for wi in waterIndexes:
                worsica_leakdetection.calculate_water_leak_second_deriv(
                    WORKSPACE_PATH + '/' + args[4] + '/' + args[1] + '_' + wi + '_anomaly' + '.tif',
                    WORKSPACE_PATH + '/' + args[4])
        else:
            # ./merged_resampled_xxxxxxxxx/merged_resampled_xxxxxxxxx_wi[_anomaly].tif
            os.makedirs(WORKSPACE_PATH + '/' + args[4], exist_ok=True)
            for wi in waterIndexes:
                worsica_leakdetection.calculate_water_leak_second_deriv(
                    WORKSPACE_PATH + '/' + args[1] + '/' + args[1] + '_' + wi + '.tif', WORKSPACE_PATH + '/' + args[4])
                copyfile(
                    WORKSPACE_PATH +
                    '/' +
                    args[1] +
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 5:
        print(
            "Usage: ./worsica_sentinel_script_v5_waterleak_calculating_second_deriv.py [MERGED_IMAGESET_NAME] [WATER_INDEX] [FROM_ANOMALY] [LD_IMAGE_NAME]")
        exit(1)
    else:
        run_script(sys.argv)
        exit(0)
